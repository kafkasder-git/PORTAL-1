...
import { TestDataManager, loginAsAdmin, loginAsUser, getUniqueTestData } from './test-utils';
...
test.describe('User Management', () => {
  ...
  test('should edit user successfully', async ({ page }) => {
    ...
    if (await editButton.isVisible()) {
      ...
      // Should show success toast
      await expect(page.locator('text=/güncellendi|updated|success/i')).toBeVisible({ timeout: 5000 });
  ...
  test('should show delete confirmation dialog', async ({ page }) => {
    ...
    if (await deleteButton.isVisible()) {
      ...
      await expect(page.locator('[role="alertdialog"], [role="dialog"]').filter({ hasText: /Sil|Delete/i })).toBeVisible();
  ...
  test('should not allow deactivating self', async ({ page }) => {
    ...
    // Find current user row (admin@test.com)
    const currentUserRow = page.locator('tr, [data-row]').filter({ hasText: /admin@test.com/i }).first();
  ...
  test('should handle pagination', async ({ page }) => {
    ...
    const nextButton = page.locator('button').filter({ hasText: /Sonraki|Next/i }).first();
  ...
  test('should disable previous button on first page', async ({ page }) => {
    ...
    // Check if pagination exists
    const prevButton = page.locator('button').filter({ hasText: /Önceki|Previous/i }).first();
  ...
  test('should show user table columns', async ({ page }) => {
    ...
    await expect(page.locator('th, [role="columnheader"]').filter({ hasText: /Ad|Name/i })).toBeVisible();
    await expect(page.locator('th, [role="columnheader"]').filter({ hasText: /E-posta|Email/i })).toBeVisible();
    await expect(page.locator('th, [role="columnheader"]').filter({ hasText: /Rol|Role/i })).toBeVisible();
    await expect(page.locator('th, [role="columnheader"]').filter({ hasText: /Durum|Status/i })).toBeVisible();
  });
