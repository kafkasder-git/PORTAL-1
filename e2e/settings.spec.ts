...
test.describe('Settings Management', () => {
  ...
  test('should display settings page for authorized users', async ({ page }) => {
    ...
    const hasInitialize = await page.locator('[data-testid="settings-initialize-button"]').isVisible();
  ...
  test('should save settings successfully', async ({ page }) => {
    ...
    if (await nameInput.isVisible()) {
      ...
      await expect(page.locator('text=/başarı|success|kaydedildi/i')).toBeVisible({ timeout: 5000 });
    }
  });
  ...
  test('should initialize settings if not exists', async ({ page }) => {
    ...
    if (await initButton.isVisible()) {
      ...
      await expect(page.locator('text=/başarı|success|oluşturuldu/i')).toBeVisible({ timeout: 5000 });
