...
export default function BulkMessagingWizard() {
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Textarea
                          id="content"
                          placeholder={messageType === 'sms' ? 'SMS mesajınızı yazın...' : 'E-posta içeriğinizi yazın...'}
                      ...
                      {messageType === 'sms' && (
                        <div className="flex items-center justify-between text-sm text-muted-foreground">
                          <span>Karakter: {messageData.content.length}</span>
                          <span>SMS: {smsMessageCount}</span>
                        </div>
                      )}
                    </CardContent>
                  </Card>
                      ...
                      <div className="space-y-3">
                        <Button
                          ...
                          onClick={() => setMessageData(prev => ({
                            ...prev,
                            content: 'Sayın [İsim], derneğimiz tarafından yardım başvurunuz değerlendirilmiştir. Detaylar için lütfen bizimle iletişime geçiniz.'
                          }))}
                        >
                        ...
                        <Button
                          ...
                          onClick={() => setMessageData(prev => ({
                            ...prev,
                            content: 'Sayın [İsim], bağışınız için teşekkür ederiz. Derneğimiz faaliyetleri için değerli katkınız bizim için önemlidir.'
                          }))}
                        >
                          <FileText className="h-4 w-4 mr-2" />
                          Bağış Teşekkürü
                        ...
                        <Button
                          ...
                          onClick={() => setMessageData(prev => ({
                            ...prev,
                            content: 'Toplantı hatırlatması: [Tarih] tarihinde [Saat]\'te [Konum] adresinde toplantımız yapılacaktır.'
                          }))}
                        >
                          <FileText className="h-4 w-4 mr-2" />
                        ...
                        <Button
                          ...
                          onClick={() => setMessageData(prev => ({
                            ...prev,
                            content: 'Derneğimizin yeni dönem faaliyetleri hakkında bilgi almak için web sitemizi ziyaret edebilirsiniz.'
                          }))}
                        >
                          <FileText className="h-4 w-4 mr-2" />
                          Genel Duyuru
            ...
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Mesajı göndermek istediğiniz alıcıları seçin
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Card className="cursor-pointer transition-all hover:shadow-md"
                          onClick={() => setSelectedRecipients(['beneficiaries'])}>
                      <CardContent className="pt-6 text-center">
                        <Users className="h-8 w-8 mx-auto mb-3 text-blue-600" />
                        <h3 className="font-semibold mb-1">İhtiyaç Sahipleri</h3>
                        <p className="text-sm text-muted-foreground">Kayıtlı ihtiyaç sahipleri</p>
                        <Badge variant="secondary" className="mt-2">
                          {selectedRecipients.includes('beneficiaries') ? 'Seçildi' : 'Seç'}
                        </Badge>
                      </CardContent>
                    </Card>

                    <Card className="cursor-pointer transition-all hover:shadow-md"
                          onClick={() => setSelectedRecipients(['donors'])}>
                      <CardContent className="pt-6 text-center">
                        <Heart className="h-8 w-8 mx-auto mb-3 text-green-600" />
                        <h3 className="font-semibold mb-1">Bağışçılar</h3>
                        <p className="text-sm text-muted-foreground">Aktif bağışçılar</p>
                        <Badge variant="secondary" className="mt-2">
                          {selectedRecipients.includes('donors') ? 'Seçildi' : 'Seç'}
                        </Badge>
                      </CardContent>
                    </Card>

                    <Card className="cursor-pointer transition-all hover:shadow-md"
                          onClick={() => setSelectedRecipients(['members'])}>
                      <CardContent className="pt-6 text-center">
                        <Building2 className="h-8 w-8 mx-auto mb-3 text-purple-600" />
                        <h3 className="font-semibold mb-1">Üyeler</h3>
                        <p className="text-sm text-muted-foreground">Dernek üyeleri</p>
                        <Badge variant="secondary" className="mt-2">
                          {selectedRecipients.includes('members') ? 'Seçildi' : 'Seç'}
                        </Badge>
                      </CardContent>
                    </Card>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-4">
                      <div className="flex-1">
                        <Label htmlFor="manual-search">Manuel Alıcı Ekle</Label>
                        <Input
                          id="manual-search"
                          placeholder={messageType === 'sms' ? 'Telefon numarası girin (örn: 05551234567)' : 'E-posta adresi girin'}
                          className="mt-1"
                        />
                      </div>
                      ...
                      <div className="flex items-center justify-between p-4 bg-muted rounded-lg">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium">
                            {selectedRecipients.length === 1 && selectedRecipients[0] === 'beneficiaries' && 'Tüm İhtiyaç Sahipleri'}
                            {selectedRecipients.length === 1 && selectedRecipients[0] === 'donors' && 'Tüm Bağışçılar'}
                            {selectedRecipients.length === 1 && selectedRecipients[0] === 'members' && 'Tüm Üyeler'}
                          </span>
                        </div>
                        <Badge variant="default">
                          {selectedRecipients.length === 1 && selectedRecipients[0] === 'beneficiaries' && '~150 alıcı'}
                          {selectedRecipients.length === 1 && selectedRecipients[0] === 'donors' && '~85 alıcı'}
                          {selectedRecipients.length === 1 && selectedRecipients[0] === 'members' && '~25 alıcı'}
                        </Badge>
                      </div>
                    )}
                  </div>
